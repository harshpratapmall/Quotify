...
| GET | `/api/v1/employees/attendance` | Owner-scoped monthly attendance register |
| PUT/DELETE | `/api/v1/employees/:id/attendance/:date` | Set or clear manager-recorded daily attendance |
...
Attendance uses `EmployeeAttendance` with `attendance_id, owner_id, employee_id, attendance_date, status, created_at, updated_at` and append-only `EmployeeAttendanceActivity` with `activity_id, owner_id, attendance_id, employee_id, attendance_date, action, previous_status, new_status, actor_id, occurred_at`. Managers may record only `present`, `absent`, `half_day`, `paid_leave`, or `unpaid_leave` for the most recent 10 Asia/Kolkata calendar days, including today. Attendance is owner-scoped, independent from payroll, and does not affect salary, payslips, PDFs, or public shares.
...
- The employees area keeps team contact details, manager-only payroll, and manager-recorded attendance. Each employee profile records calendar-month salary, payments, advances, credits, deductions, due balance, and calculated payment status. Inactive staff retain history but cannot start a new salary month or receive new attendance. Attendance is not payroll proration; statutory deductions and employee self-service are not implemented.
