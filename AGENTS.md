...
- Employee rows use `Employee!A:K`: `employee_id, owner_id, name, phone, email, address, designation, notes, status, created_at, updated_at`. Payroll adds `EmployeePayroll!A:G`: `payroll_id, owner_id, employee_id, period, base_salary, created_at, updated_at`, and `EmployeePayrollEntries!A:L`: `entry_id, owner_id, employee_id, period, type, amount, entry_date, label, note, recovery_period, created_at, updated_at`. All rows are owner-scoped. Monthly status is derived from salary, credits, deductions, advances, and payments; advances carry into later salary months when not fully recovered. Inactive employees retain history but cannot create a new salary month. Attendance, leave, statutory payroll, and employee self-service are absent.
