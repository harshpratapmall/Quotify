...
- Employee rows use `Employee!A:K`: `employee_id, owner_id, name, phone, email, address, designation, notes, status, created_at, updated_at`. Payroll adds `EmployeePayroll!A:G`, `EmployeePayrollEntries!A:L`, `EmployeePayrollStates!A:H` for finalized/open month locks, and append-only `EmployeePayrollActivity!A:J` audit events. All rows are owner-scoped. Advance recovery cannot precede its issue month and unpaid recovery carries into the next available salary month. Employees with payroll history cannot be deleted; set them inactive instead. Inactive employees retain history but cannot create a new salary month. Attendance, leave, statutory payroll, and employee self-service are absent.
