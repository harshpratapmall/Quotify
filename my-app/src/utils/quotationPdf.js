...
function icon(pdf, type, x, y, size = 6) {
...
export async function generateQuotationPdf({ quotation, items, includeGst, gstPercentage,
  subtotal, tax, total, activeQuotationId, logoSource, businessProfile = {} }) {
  ...
  const reference = `${businessProfile.quotePrefix || 'QUOTE'}-${clientInitials(quotation.clientName)}-${date.replaceAll('-', '')}-${suffix}`;
  const months = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'];
  const client = String(quotation.clientName || 'client').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'client';
  const filename = `quotation-${client}-${date.slice(8, 10)}${months[Number(date.slice(5, 7)) - 1]}.pdf`;
  pdf.setProperties({ title: `Quotation ${reference}` });
  ...
  const footer = footerFields.map(([type, value, part]) => {
    ...
    return { type, width, lines: wrap(value, width - 14, 7.2) };
  });
  const footerHeight = Math.max(20, ...footer.map((entry) => entry.lines.length * 3.2 + 10));
  const footerTop = 289 - footerHeight;
  if (footerHeight > 90) throw new Error('Business contact details are too long for the quotation footer. Please shorten them in Business Profile.');
  const pageBottom = footerTop - 7;
  const decoration = () => {
    pdf.setFillColor('#2c8062');
    pdf.rect(MARGIN, 6, WIDTH, 4, 'F');
    pdf.setFillColor(GREEN);
    pdf.lines([[30, 0], [0, 4], [-34, 0], [4, -4]], 174, 6, [1, 1], 'F', true);
    pdf.setFillColor('#eaf4f0');
  ...
  const compactHeader = () => {
    text('QUOTATION', MARGIN, 21, 18, true, GREEN);
  ...
  text('QUOTATION', 127, 31, 28, true, GREEN);
  text('Quote No.', 127, 39, 10, false, GREEN);
  text(refLines, 155, 39, 9);
  const dateY = 39 + refLines.length * 3.7 + 3;
  text('Date of Issue', 127, dateY, 10, false, GREEN);
  ...
  clientFields.forEach((entry, index) => {
    ...
    pdf.setFillColor('#e0efea');
    pdf.roundedRect(clientX + 4, clientTop + 5, 11, 11, 2, 2, 'F');
    icon(pdf, entry.type, clientX + 6.5, clientTop + 7.5);
  ...
  autoTable(pdf, {
    ...
    headStyles: { fillColor: GREEN, textColor: '#ffffff', fontStyle: 'bold', minCellHeight: 14 },
    alternateRowStyles: { fillColor: '#f3f8f7' },
  ...
  const panelHeight = includeGst ? 43 : 32;
  ...
  text('TOTAL ESTIMATE', 106, totalTop + panelHeight - 6, 12, true, '#ffffff');
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    footer.forEach((entry, index) => {
      if (index) { pdf.setDrawColor(RULE); pdf.setLineWidth(0.25); pdf.line(x, footerTop + 5, x, 286); }
      icon(pdf, entry.type, x + 2, footerTop + 7, 6);
      text(entry.lines, x + 11, footerTop + 8, 7.2);
      x += entry.width;
    });
    pdf.setFillColor('#2c8062');
    pdf.lines([[28, 0], [0, 4], [-32, 0], [4, -4]], 176, 287, [1, 1], 'F', true);
    if (pageCount > 1) text(`Page ${page} of ${pageCount}`, 105, 294, 7, false, GREEN, { align: 'center' });
  }
  return { pdf, filename };
}
