  ...
  return (
    <main className="public-share-page">
      <section className="public-share-card">
        ...
        <div className="public-share-total"><span>Subtotal <strong>{currency(share.subtotal)}</strong></span>{share.includeGst && <span>GST ({share.gstRate}%) <strong>{currency(share.tax)}</strong></span>}<span className="grand-total">Total <strong>{currency(share.total)}</strong></span>{share.documentType === 'bill' && share.paymentStatus === 'partially_paid' && (() => { const { received, pending } = getPaymentSummary(parsePayments(share.payments), share.total); return <span className="bill-payment-summary">Received <strong>{currency(received)}</strong> · Pending <strong>{currency(pending)}</strong></span>; })()}</div>
