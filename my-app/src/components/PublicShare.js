...
function PublicShare({ token }) {
  ...
  if (error) return <main className="public-share-page"><section className="public-share-card"><p className="eyebrow">Shared document</p><h1>Link unavailable</h1><p>{error}</p></section></main>;
  if (!share) return <main className="public-share-page"><section className="public-share-card"><p className="eyebrow">Shared document</p><h1>Loading document...</h1></section></main>;
  ...
  return (
    <main className="public-share-page">
      <section className="public-share-card">
        <header className="public-share-header"><div>{share.business?.logoUrl && <img src={share.business.logoUrl} alt="" />}<div><p className="eyebrow">{share.documentType}</p><h1>{share.business?.businessName || 'Quotation'}</h1></div></div><span>{share.date}</span></header>
        ...
        <div className="public-share-items">{items.filter((item) => item.description || item.rate).map((item, index) => <div className="public-share-item" key={`${item.description}-${index}`}><span>{item.description || 'Item'}</span><span>{item.quantity || 0} x {currency(Number(item.rate) || 0)}</span><strong>{currency((Number(item.quantity) || 0) * (Number(item.rate) || 0))}</strong></div>)}</div>
