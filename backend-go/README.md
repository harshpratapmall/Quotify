...
- Quotation and bill sharing both support POST to create and DELETE to revoke `/api/v1/{quotations|bills}/:id/share`. Public links use `GET /api/v1/public/share/:token`; bill share responses include `paymentStatus` and the raw `payments` array so the share page renders the same Received/Pending summary as the app. Share links expire 30 days after creation and return `410 Gone` once expired or revoked; `ShareLinks` timestamps are stored as Asia/Kolkata `DD-MM-YYYY HH:MM:SS`.
