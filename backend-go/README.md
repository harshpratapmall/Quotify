...
- Employees: `GET/POST /api/v1/employees`, `GET/PUT/DELETE /api/v1/employees/:id`, an owner-scoped payroll register/export, and monthly payroll endpoints beneath `/api/v1/employees/:id/payroll`. Payroll records calendar-month base salary and editable credit, deduction, advance, and payment entries. Finalized months reject edits until reopened; state and append-only activity live in separate Sheets tabs. Status is calculated from monetary totals.
